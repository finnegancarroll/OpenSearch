 ...
 * @opensearch.internal
 */
final class ProtobufFetchSearchPhase extends SearchPhase {
    ...
    ProtobufFetchSearchPhase(
        ...
        this.progressListener = context.getProtobufTask().getProgressListener();
    }
    ...
    public void run() {
        context.execute(new AbstractRunnable() {
            ...
            public void onFailure(Exception e) {
                context.onPhaseFailure(ProtobufFetchSearchPhase.this, "", e);
            }
        });
    }
            ...
            if (scoreDocs.length == 0) {
                ...
                for (int i = 0; i < docIdsToLoad.length; i++) {
                    ...
                    if (entry == null) { // no results for this shard ID
                        ...
                        ProtobufShardFetchSearchRequest fetchSearchRequest = createFetchRequest(
                            queryResult.queryResult().getContextId(),
                            i,
                            entry,
                            lastEmittedDocPerShard,
                            searchShardTarget.getOriginalIndices(),
                            queryResult.getProtobufShardSearchRequest(),
    ...
    protected ProtobufShardFetchSearchRequest createFetchRequest(
        ...
        return new ProtobufShardFetchSearchRequest(
            originalIndices,
            contextId,
            shardSearchRequest,
            entry,
    ...
    private void executeFetch(
        ...
        final ProtobufShardFetchSearchRequest fetchSearchRequest,
        final QuerySearchResult querySearchResult,
        final Transport.Connection connection
    ) {
        context.getSearchTransport()
            .sendExecuteFetchProtobuf(
                connection,
                fetchSearchRequest,
                context.getProtobufTask(),
