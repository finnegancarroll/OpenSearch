...
public enum ScalarFunction {
    ...
    /**
     * Calcite's {@code ARRAY_CONTAINS} — element membership over an array. The PPL frontend
     * rewrites implicit {@code multi_value = x} / {@code != x} into this so it operates on the
     * array elements. Registered with its reference operator so the filter rule recognizes it as a
     * predicate; the DataFusion backend maps it to native {@code array_has}.
     */
    ARRAY_CONTAINS(
        Category.SCALAR,
