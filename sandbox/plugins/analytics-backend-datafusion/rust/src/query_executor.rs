...
async fn build_dataframe(
    ...
    let logical_plan = crate::substrait_consumer::from_substrait_plan(&ctx.state(), &substrait_plan).await?;
...
pub async fn execute_with_context(
    ...
    let query_future = async {
        ...
        let logical_plan = crate::substrait_consumer::from_substrait_plan(&handle.ctx.state(), &substrait_plan).await?;
        log_debug!(
