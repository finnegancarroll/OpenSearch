...
public class NestedQueryRecursionLimitIT extends AnalyticsRestTestCase {
    ...
    public void testDeeplyNestedQueryReturns400() throws Exception {
        ...
        // 30 nested calls should succeed
        String shallow = buildNestedExpr(30);
        Request okRequest = new Request("POST", "/_analytics/ppl");
        okRequest.setJsonEntity("{\"query\": \"source=" + INDEX + " | eval result = " + shallow + "\"}");
        Response okResponse = client().performRequest(okRequest);
        assertEquals(200, okResponse.getStatusLine().getStatusCode());
